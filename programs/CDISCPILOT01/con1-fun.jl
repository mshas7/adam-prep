...
function find_previous_nominal_dose(adpc::DataFrame, ex_exp::DataFrame)
    ...
    for (i, row) in enumerate(eachrow(adpc))
        usubjid = row.USUBJID
        curr_nfrlt = row.NFRLT
        key = (usubjid,)

        if haskey(ex_by_usubjid, key)
            subject_doses = ex_by_usubjid[key]
            previous = subject_doses[subject_doses.NFRLT .< curr_nfrlt, :]

            if !isempty(previous)
                idx = argmax(previous.NFRLT)
                last_nom = previous[idx, :]

                prev_rows[i] = Dict(
                    :NFRLT_prev => last_nom[:NFRLT]
                )
            else
                prev_rows[i] = Dict(:NFRLT_prev => missing)
            end
        else
            prev_rows[i] = Dict(:NFRLT_prev => missing)
        end
...
function compute_bmi(; height, weight)
    if ismissing(height) || ismissing(weight) || height <= 0
        return missing
    end
    # height in cm → meters
...
function compute_bsa(; height, weight, method = "Mosteller")
    if ismissing(height) || ismissing(weight) || height <= 0 || weight <= 0
        return missing
    end
...
function compute_egfr(; creat, creatu, age, weight, sex, method)
    if any(ismissing.((creat, age, sex)))
        return missing
    end

    scr = creatu == "SI" ? creat / 88.4 : creat
