...
"""
    table_one_mod(table, analyses; keywords...)
...
function table_one_mod(
    ...
    if sort && !isempty(groupsymbols)
        try
            Base.sort!(df, groupsymbols)
        catch e
            throw(Base.SortingError())
        end
    end
...
"""
    table_one_mod(table; kwargs...)

Create a `table_one_mod` with with all columns from `table` except those used in the `groupby` keyword.
"""
function table_one_mod(table; groupby = [], kwargs...)
    groups = make_groups(groupby)
    groupsyms = [g.symbol for g in groups]
    all_names = Tables.columnnames(table)
    all_names_but_groups = setdiff(all_names, groupsyms)
    return table_one_mod(table, all_names_but_groups; groupby, kwargs...)
end
